...
```bash
$VP -m mnemonica.ui.app --session sessions/phase3/session.json --extraction sessions/phase3/extraction.json --audio "sessions/phase3/MedScribe test 1.m4a"
```
...
```bash
$VP -m mnemonica.audio.pipeline path/to/visit.m4a --session-dir sessions/mine --enrollment path/to/clinician.wav --out sessions/mine/session.json
```

```bash
$VP -m mnemonica.verify.run sessions/mine/session.json -o sessions/mine/extraction.json
```

```bash
$VP -m mnemonica.ui.app --session sessions/mine/session.json --extraction sessions/mine/extraction.json --audio path/to/visit.m4a
```
...
### Test without recording anything

Recording two people every time you want to check a template is a slow loop.
`from_script` builds the same `Session` contract Track B emits, from a
role-play script, with no audio and no models:

```bash
$VP -m mnemonica.audio.from_script fixtures/roleplay_script_2.md --session-dir sessions/script2 --out sessions/script2/session.json
```

```bash
$VP -m mnemonica.verify.run sessions/script2/session.json -o sessions/script2/extraction.json
```

Then point the UI at those two files. About 60 s, all of it the LLM.

It fabricates three things, each load-bearing somewhere: **timings** are
synthesised (so gate B5's listening check cannot be done this way),
**probabilities are all 1.0** — a script cannot mumble, so D16 category 2 can
never fire from one — and **roles** come from the `DR:`/`PT:` labels rather
than voice enrollment, bypassing D20 and B6. Offsets are real and the
`Session` validator enforces them as usual.

### A PDF of the patient's page

The approved document is saved as HTML. Chrome renders it through the real
`@media print` rules:

```bash
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless --disable-gpu --no-pdf-header-footer --print-to-pdf="$PWD/visit-summary.pdf" "file://$PWD/$(ls -t data/records/*/visit-summary.html | head -1)"
```
...
296 tests, about two seconds. They cover logic only — no model inference — so
...
```bash
$VP -m mnemonica.ui.app --sweep          # delete unapproved sessions >24h old
$VP -m mnemonica.kb.build                # rebuild rxnorm.db from rrf/
$VP -m mnemonica.kb.openfda              # rebuild openfda_labels.db
...
| File | What it is |
|---|---|
| [SPEC.md](SPEC.md) | The 28 architecture decisions (D1–D28), each with its reasoning and the alternatives rejected |
...
mnemonica/
  ...
  audio/         Whisper + pyannote -> Session; from_script for a text-only Session
